...
type LiProps = {
  ...
  hoverTab: string;
  setHoverTab: Dispatch<SetStateAction<string>>;
};

const Li = ({ item, hoverTab, setHoverTab }: LiProps) => {
  const { id, label } = item;
  ...
  return (
    ...
    >
      <Link
        className="-mx-3 relative flex rounded-custom px-3 py-2 text-primary/70 capitalize"
        href={`#${id}`}
        prefetch={false}
      >
        {hoverTab === id ? (
